...
Server::Server(ServerContext* server_context) {
	...
	while (map_it != map_ite) {
		...
		while (ports_it != ports_ite) {
			...
			{
				in_addr_t host_addr = _getHostInetAddrFromStr((*map_it).first);
...
in_addr_t Server::_getHostInetAddrFromStr(const std::string& host_str) {
    in_addr_t host_addr;
    if (host_str == "*") {
...
const std::list<Listener*> & Server::getListeners(void) const { return _listeners; };
